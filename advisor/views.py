...
from .serializer import (
    AdvisorDashboardSerializer,
    JobAssignBaySerializer,
    MakeOnlineSerializer,
    SerivesCurrentSerializer,
)
...
class CurrentJobs(AutoPrefetchViewSetMixin, APIView):
    def get(self, request, advisor_id):
        jobs = Services.objects.filter(
            Q(advisor=advisor_id) & ~Q(status="finished"), ~Q(status="billing")
        )
...
class JobAssignerToBay(APIView):
    def post(self, request):
        ...
        if serializer_obj.is_valid():
            ...
            Services.objects.filter(id=request.data["current_job"]).update(
                status="in-bay"
            )
...
class AdvisorDashboard(APIView):
    ...
    def get(self, request):
        ...
        # ongoing_jobs
        # free_bays
        service_done = Services.objects.filter(
            Q(status="finished") & Q(advisor=request.user)
        ).count()

        ongoing_jobs = Services.objects.filter(
            Q(status="in-bay") & Q(advisor=request.user)
        ).count()
