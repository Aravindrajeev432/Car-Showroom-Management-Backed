...
def CarNameFile(instance, filename):
    return "/cars".join(
        ["images/cars", str(instance.model_name + instance.model_year), filename]
    )
...
def nameFile(instance, filename):
    return "/cars".join(
        ["images", str(instance.model_name + instance.model_year), filename]
    )
...
class DisplayCarImages(models.Model):
    car = models.ForeignKey(
        DisplayCars, null=True, related_name="carimg", on_delete=models.CASCADE
    )  # django-doctor: disable=field-null-not-blank
    image = models.ImageField(
        upload_to=namePicFile,
        blank=True,
        null=True,
        validators=[FileExtensionValidator(allowed_extensions=["jpg", "jpeg", "webp"])],
    )
