...
class AddManager(APIView):
    def get(self, request):
        post = {"phone_number": "", "username": "", "password": "", "email": ""}
...
class AddEmployee(MycustomMixin, APIView):
    ...
    def post(self, request):
        ...
        if Account.objects.filter(
            Q(phone_number=body["phone_number"]) & Q(is_staff=True)
        ).exists():
        ...
        if body["role"] == "Advisor":
            ...
            Account.objects.create_front_desk(
                email=email,
                username=username,
            ...
            Account.objects.create_mechanic(
                email=email,
                username=username,
                phone_number=phone_number,
                password=password,
        ...
        return Response(
            status=status.HTTP_502_BAD_GATEWAY, statusText="email already taken"
        )
